...
import { ERC20NameBytes } from '../types/IPWorld/ERC20NameBytes'
import { ERC20SymbolBytes } from '../types/IPWorld/ERC20SymbolBytes'
import { isNullIPValue } from '.'
...
export function fetchTokenSymbol(tokenAddress: Address, tokenOverrides: StaticTokenDefinition[]): string {
  ...
  if (symbolResult.reverted) {
    ...
    if (!symbolResultBytes.reverted && !isNullIPValue(symbolResultBytes.value.toHexString())) {
      // for broken pairs that have no symbol function exposed
      if (!isNullIPValue(symbolResultBytes.value.toHexString())) {
...
export function fetchTokenName(tokenAddress: Address, tokenOverrides: StaticTokenDefinition[]): string {
  ...
  if (nameResult.reverted) {
    ...
    if (!nameResultBytes.reverted) {
      ...
      if (!isNullIPValue(nameResultBytes.value.toHexString())) {
